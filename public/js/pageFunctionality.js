...
function getGraph() {
    $.get('https://arvind.io:8085/getevents', events => {
        ...
        for (var key in events) {
            events[key].oseconds = Math.abs(events[key].from - floor) + 6 + Math.abs(events[key].to = events[key].from);
            floor = events[key].to;
        }
        ...
        for (var key in events) {
            events[key].pseconds = Math.abs(events[key].from - floor) + 6 + Math.abs(events[key].to = events[key].from);
            floor = events[key].to;
        }
